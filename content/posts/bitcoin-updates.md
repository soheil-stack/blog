...
draft = true
title = 'How Bitcoin Updates Its Consensus Rules'
+++

## Introduction

Bitcoin is an open-source project. Although [Bitcoin Core](https://github.com/bitcoin/bitcoin) is written primarily in C++, there are also independent implementations of the Bitcoin protocol in other languages, such as [btcd](https://github.com/btcsuite/btcd), a Go implementation. 
In this article, we will use Go and `btcd` to explore how Bitcoin updates its consensus rules.

Bitcoin is a decentralized network of independently operated nodes, and there is no central authority forcing every node to run the same implementation. Each node can choose which software to run and can update that software independently.
However, not every change a node makes to its software affects the Bitcoin network in the same way.

A node can modify its local policies without changing the consensus rules. For example, a node could choose not to relay or accept in its mempool transactions with very low fees.

But when it comes to consensus rules, things change. Consensus rules define which transactions and blocks are considered valid by a node. For example a rule that limits the maximum size of a block is a consensus rule.

If different nodes enforces different consensus rules, they may disagree about a prticular block is valid. One node may accept a block while another rejects it. This can lead the nodes to follow different chains and can ultimately lead to a **fork**.

## Bitcoin Forks

There are two types of fork, **hard forks** and **soft forks**.
...
With a soft fork, the new rules are more restrictive, so blocks that follows new rules are still valid according to the old rules.
...
With a hard fork, the new rules can allow blocks and transactions that are invalid according to the old nodes. As a result, nodes running the old rules will reject blocks that are valid under the new rules.
...
The rule was initially applied to all blocks whose timestamp was after March 12, 2012, 00:00 UTC. It was later changed so that the rule applied to all blocks, with the exception of two historical blocks.

This approach has an important limitation. The activation has to be set sufficiently far in the future to give nodes enough time to upgrade, because there is no on-chain indication that a sufficient number of nodes have upgraded before the deadline.
...
However, the signaling itself was not the activation mechanism. BIP16 still used a predetermined activation date. On April 1, 2012, upgraded nodes would begin enforcing the new P2SH rules if sufficient miner support had been observed.
...
By commiting the block height into the coinbase transaction, it prevented duplicate coinbase transactions from occurring as different heights and fixed transaction ID issues.
...
Every block header already contains a version field, so instead of using an arbitary string in the coinbase transaction like BIP16, miners signaled support for BIP34 by setting the block version to `2`.
...
First, when 750 out of the previous 1,000 blocks singled version `2`, upgraded nodes began requiring version `2` blocks to follow the new BIP34 rules, while still accepting version `1` blocks.
...
With BIP34 the entire block version was used to signal an upgrade. This create a problem when multiple soft forks need to be activated independently.
For example:

```text
version 2 -> Soft Fork A -> not yet acticated
version 3 -> Soft Fork B -> ready for activation
```

If miners set the block version to `3` to signal support for Soft Fork B, nodes cannot distinguish wheher this means:
...
The deployment can move through different states during its lifecycle and can eventually reach either `ACTIVE` if activation succeeds or `FAILED` is the activation conditions are not met before the timeout.
...
First, make sure you have `git` and `go` installed on your computer and then run the following commands:

```sh
git clone http://github.com/soheil-stack/versionbits
cd versionbits
git run . 
```
...
Our project defines seperate deployments for CSV, SegWit, and Taproot, each with its own signaling bit and activation window.
...
Block timestamps are provided by miners and are not a reliable source of time. A miner has some flexibility when setting the timestamp of a block, which means indiviual block timestamps can be inaccurate.
To avoid this problem, BIP9 uses **Median Time Past (MTP)** instead of the timestamp of the current block.

Median Time Past is calculated from the timestamp of the current block and the 10 preceding blocks, for a total of 11 blocks. The timstamps are sorted, and the middle value is used as the effective time.
...
func calcMedianTimePast(headers []*wire.BlockHeader, height int) time.Time {
	...
	// number of available timestamps will be fewer than desired near the beginning of the block chain
	i := 0
...
These methods are used during deployment state transitions.
...
To distinguish BIP9 signaling from other uses of the version field, BIP9 requires the first three bits of the version field to be `001`.
...
BIP9 uses the three most significant bits of the block version as a marker. The remaining 29 bits, numbered 0 through 28, are available for signaling indiviual soft fork deployments.
...
- a deplyment has a start time and an end time,
...
The remaining question is how these pieces are combined to determine the current state of the development.
...
- `STARTED` -- the deployment is active and nodes are counting miner signaling.
...
On Bitcoin mainnet, a confirmation window contains **2,016 blocks**, and at least **1,916 blocks** must signal support for the development.
...
After the required delay, the deployment becomes `ACTIVE`.
...
```sh
2026/08/11 17:18:01 connecting to 119.196.44.37:8333
2026/08/11 17:18:01 -> version
2026/08/11 17:18:02 <- version
2026/08/11 17:18:02 received unknown message
2026/08/11 17:18:02 <- verack
2026/08/11 17:18:02 -> getheaders
2026/08/11 17:18:02 received unknown message
2026/08/11 17:18:02 <- ping
2026/08/11 17:18:02 -> pong
2026/08/11 17:18:03 <- headers (833)
2026/08/11 17:18:03 header synchronization complete
2026/08/11 17:18:03 loading deployment caches from data/deployment.cache
2026/08/11 17:18:03 loaded deployment caches
2026/08/11 17:18:03 loading headers from data/headers.dat
2026/08/11 17:18:06 loaded 962013 headers
2026/08/11 17:18:06 evaluating deployment states at height 962012
2026/08/11 17:18:06 CSV      -> ThresholdActive
2026/08/11 17:18:06 Segwit   -> ThresholdActive
2026/08/11 17:18:06 Taproot  -> ThresholdActive
2026/08/11 17:18:06 write deployment caches to data/deployment.cache
```
